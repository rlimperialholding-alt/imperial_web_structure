...
const screenshotRoot = path.join(root, 'screenshots', 'technology-pages');
const renderedRoot = path.join(root, 'rendered', 'technology-pages');
const reportPath = path.join(root, 'technology-pages-report.json');
const routes = ['/mibol-epuljon','/technologiak/favazas','/technologiak/tegla','/technologiak/liapor','/technologiak/acelszerkezet'];
const viewports = [
  { name: 'mobile', width: 390, height: 844 },
  { name: 'tablet', width: 768, height: 1024 },
  { name: 'desktop', width: 1440, height: 1000 },
];
const forbidden = ['látható karakter','önálló fejezet','szakmai kérdés','QA kötelez','NIM CMS','projektidővonal','döntési brief','konverziós réteg'];
...
(async () => {
  ...
  for (const route of routes) {
    for (const viewport of viewports) {
      ...
      await page.waitForSelector('.technology-page', { timeout: 30000 });
      ...
      const dom = await page.evaluate(forbiddenPhrases => {
        ...
        const clipped = [...document.querySelectorAll('main h1, main h2, main h3, main p, main li, main summary, main .button, main span, main strong')].filter(el => {
        ...
        return {
          ...
          technologyPages: document.querySelectorAll('.technology-page').length,
          faqItems: document.querySelectorAll('.technology-faq details').length,
          layoutSignature: [...article.classList].find(name => name.startsWith('technology-page--')),
          backgroundImage: getComputedStyle(document.querySelector('.technology-hero__image')).backgroundImage,

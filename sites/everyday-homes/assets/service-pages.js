const SERVICE_PAGES = {
  ...
  "/mi-intezzuk/finanszirozas": {
    ...
    secondary: ["Megnézem a teljes projektkeretet", "/szamolok/teljes-projektkeret"],
    ...
    stages: [
      ...
      ["A saját forrás valódi térképe", "Nem minden megtakarítás költhető el az első munkaszakaszban; foglaló, tervezés, közmű és átmeneti lakhatás is kérhet pénzt.", "Ti feltárjátok a rendelkezésre állás időpontját és a nem felhasználható vésztartalékot; mi idővonalra tesszük a feladatokat.", "Forrástábla különíti el a telket, az építést, a járulékos tételeket és a biztonsági tartalékot.", "A bankszámlán látható teljes összeg nem azonos a kockázat nélkül elkölthető önerővel."],
      ["Teljes projekt, nem csak házár", "A telek, alapozási eltérés, közmű, külső munka, díjak, költözés és átmeneti költségek a ház csomagárán kívül is megjelenhetnek.", "Ti megadjátok a projekt határait; mi tételesen szétválasztjuk a vállalt, becsült, telekfüggő és külön megrendelhető részeket.", "Alsó, várható és tartalékkal növelt projektkeret készül, a bizonytalanságok megnevezésével.", "A legkedvezőbb házár félrevezető, ha a befejezéshez szükséges külső tételek kimaradnak mellőle."],
      ...
      ["A bank és az építkezés ugyanazon idővonalon", "A készültségi fok, értékbecslés, számla, önerő felhasználása és folyósítás sorrendje nem válhat el a munkaszakaszoktól.", "Ti elfogadjátok a finanszírozási feltételeket; a projekt és a pénzügyi partner egyezteti, mikor milyen igazolás kell.", "Pénzügyi-kivitelezési útvonal mutatja, mely teljesítés után milyen forrás válhat elérhetővé.", "Ha a kivitelező fizetési pontja megelőzi a banki folyósítást, átmeneti forráshiány állíthatja meg a munkát."],
    ...
    proofItems: [
      ...
      ["Forrásidővonal", "Nemcsak az összeg, hanem a saját forrás és finanszírozás tényleges elérési ideje is szerepel."],
...
function serviceJourney(page) {
  return `<section id="${page.layout === "responsibility-grid" ? "felelossegi-rend" : "tervezesi-ut"}" class="service-journey service-journey--${escapeHtml(page.layout)}"><header><p class="decision-kicker">${escapeHtml(page.signature)}</p><h2>${page.layout === "funding-roadmap" ? "A pénz útja ugyanúgy tervezendő, mint a házé." : page.layout === "responsibility-grid" ? "Tíz átadási pont tartja egyben a teljes építkezést." : "Tíz állomás választja el az első beszélgetést a lezárt tervtől."}</h2></header><div class="service-journey__items">${page.stages.map((stage, index) => `<article><span>${String(index + 1).padStart(2, "0")}</span><h3>${escapeHtml(stage[0])}</h3><p>${escapeHtml(stage[1])}</p><dl><div><dt>Ti</dt><dd>${escapeHtml(stage[2])}</dd></div><div><dt>Eredmény</dt><dd>${escapeHtml(stage[3])}</dd></div><div><dt>Figyelmeztető jel</dt><dd>${escapeHtml(stage[4])}</dd></div></dl></article>`).join("")}</div></section>`;
}

function serviceFaq(page) {
  return `<section class="service-faq"><header><p class="decision-kicker">Amit érdemes még az elején tisztázni</p><h2>25 kérdés, amelyből jobb döntés születik</h2></header><div class="service-faq__items">${page.faq.map(([question, answer], index) => `<details><summary><span>${String(index + 1).padStart(2, "0")}</span>${escapeHtml(question)}</summary><p>${escapeHtml(answer)}</p></details>`).join("")}</div></section>`;
}

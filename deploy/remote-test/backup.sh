...
# Immutable source data lives under /app/data; generated contract packages,
# canonical documents and demo runtime state live on the persistent /app/runtime
# volume. Database dumps contain neither, so a release-safe backup archives both.
base_compose exec -T platform-core \
  tar -C /app -czf - data runtime \
  >"$destination/platform-core-data.tar.gz"
tar -tzf "$destination/platform-core-data.tar.gz" >/dev/null

if [[ -s "$repo_root/.source-commit" ]]; then
  cp "$repo_root/.source-commit" "$destination/source-commit.txt"
elif git -C "$repo_root" rev-parse HEAD >"$destination/source-commit.txt" 2>/dev/null; then
  :
else

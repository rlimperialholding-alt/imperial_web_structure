...
def _registry_row(db, project_id: str, responsible: str | None = None) -> None:
    if not db.scalar(select(ProjectRegistry).where(ProjectRegistry.project_id == project_id)):
        db.add(ProjectRegistry(
            project_id=project_id, name=f"{project_id} szintetikus projekt",
...
def test_allocation_project_scope_helpers_fail_closed(db):
    ...
    with pytest.raises(PermissionError):
        require_finance_plan_project_scope(
            db, _user("project-manager", "pm-a@imperial.local"), plan.plan_id
        )
    with pytest.raises(PermissionError):
        require_project_finance_scope(
            db, _user("project-manager", "pm-a@imperial.local"), "T77-SCOPE"
        )
    assert require_finance_plan_project_scope(
        db, _user("project-manager", "pm-other@imperial.local"), plan.plan_id
...
def test_concurrent_order_creation_creates_at_most_one_order(tmp_path):
    engine = create_engine(
        f"sqlite:///{(tmp_path / 'task77-orders.db').as_posix()}",
        future=True, connect_args={"timeout": 30},
    )
    Base.metadata.create_all(bind=engine)
    factory = sessionmaker(bind=engine, autoflush=False, expire_on_commit=False, future=True)
    with factory() as seed:
        seed_gate_plan(
            seed, project_id="T77-RACE", revenue="10000000",
            direct_lines=[("MAT-RACE", "6000000", "labour")],
        )
        ...
        seed.add(ProcurementRequirement(
            requirement_id="REQ-RACE", project_id="T77-RACE", category="falazat",
        ...
        seed.add(ProcurementOffer(
            offer_id="POFF-RACE", requirement_id="REQ-RACE", supplier_name="B Kft.",
        ...
        seed.add(ProcurementSelection(
            selection_id="PSEL-RACE", requirement_id="REQ-RACE", offer_id="POFF-RACE",
    ...
    def gate_hook(db, **kwargs):
        ...
        if len(calls) == 1:
            ...
            second_result["order"] = procurement_service.create_order(
                s2, data, actor="owner@imperial.local"
            )
    ...
    with factory() as check:
        orders = list(check.scalars(select(ProcurementOrderProjection).where(
            ProcurementOrderProjection.selection_id == "PSEL-RACE")).all())
        assert len(orders) == 1
...
def test_import_approval_rechecks_draft_after_lock_acquisition(db):
    row = preview_budget_import(
        db, project_id="IMP-T77", file_name="budget.csv",
        data=_csv_bytes([_row("MAT-LOCK")]), actor="fixture@imperial.local",
    )
    ...
    assert db.scalar(select(ProjectFinancePlan).where(
        ProjectFinancePlan.plan_id == plan.plan_id)).status == "draft"
    # …majd egy konkurens vezetői jóváhagyás jóváhagyottra állítja.
    with SessionLocal() as other:
        other_plan = other.scalar(select(ProjectFinancePlan).where(
            ProjectFinancePlan.plan_id == plan.plan_id))
    ...
    db.expire_all()
    fresh_plan = db.scalar(select(ProjectFinancePlan).where(
        ProjectFinancePlan.plan_id == plan.plan_id))
    assert fresh_plan.status == "approved"
    assert db.scalars(select(ProjectFinanceBudgetLine).where(
        ProjectFinanceBudgetLine.plan_id_fk == fresh_plan.id)).all() == []
    assert db.scalar(select(ProjectBudgetImport).where(
        ProjectBudgetImport.import_id == row.import_id)).status == "preview"
...
def test_clone_remaps_parent_when_child_precedes_parent_in_collection(db):
    ...
    db.add(ProjectFinanceBudgetLine(
        line_id="FIN-LINE-T77-CHILD", plan_id_fk=plan.id, cost_code="CHILD-CODE",
    ...
    db.add(ProjectFinanceBudgetLine(
        line_id="FIN-LINE-T77-PARENT", plan_id_fk=plan.id, cost_code="PARENT-CODE",
    ...
    cloned = list(db.scalars(select(ProjectFinanceBudgetLine).where(
        ProjectFinanceBudgetLine.plan_id_fk == clone.id)).all())
...
# --- AC-07: 0073 migráció kényszer-proof (statikus + futás idejű séma) ---


def test_migration_0073_adds_and_enforces_order_selection_unique_constraint(db):
    migration_path = (
        Path(__file__).resolve().parents[1]
        / "alembic" / "versions" / "20260907_0073_tender_margin_gate.py"
    )
    source = migration_path.read_text(encoding="utf-8")
    assert "_add_missing_unique_constraint" in source
    ...
    with pytest.raises(IntegrityError):
        with SessionLocal() as other:
            other.add(ProcurementOrderProjection(
                order_id="PO-T77-DUP-1", project_id="P-DUP",
                supplier_name="S-DUP", item_summary="X", status="ordered",
                selection_id="SEL-DUP",
            ))
            other.add(ProcurementOrderProjection(
                order_id="PO-T77-DUP-2", project_id="P-DUP",
                supplier_name="S-DUP", item_summary="X", status="ordered",
                selection_id="SEL-DUP",
            ))
            other.commit()

...
def test_public_new_build_calculator_uses_approved_source_and_hides_internal(client):
    ...
    data = response.json()
    assert data["estimated_net_total_huf"] == 68_000_000
    assert data["customer_facing_price_suffix"] == "+ ÁFA"
    ...
    page = client.get("/experience")
    assert page.status_code == 200
    assert "estimated_net_total_huf" in page.text
    assert "+ ÁFA" in page.text
    assert "estimated_gross_total_huf" not in page.text


def test_internal_new_build_calculator_returns_margin_gate(client):
    response = client.post(
        "/api/internal/calculators/new-build",
        json={
            "brand": "imperial",
            "technology": "Danish Fabrik",
            "completion_level": "Kulcsrakész",

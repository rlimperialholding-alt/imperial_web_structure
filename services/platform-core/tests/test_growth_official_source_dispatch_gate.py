...
def test_refresh_is_before_global_guard_and_callback_is_immediately_before_post(
    ...
    def fresh(*args, **kwargs):
        ...
        fresh_calls += 1
        events.append(
            "receipt_pre_global" if fresh_calls == 1 else "receipt_pre_post"
        )
...
def test_unproven_json_and_rss_sources_are_unaffected_and_never_live_fetched(
    ...
    assert result.status == "sent"
    assert official_runtime.loads - loads_before == 1
    assert "gmail_post" in events


def test_unproven_nonofficial_final_guard_does_not_reload_on_promotion_race(
    db,
    monkeypatch,
    official_runtime,
):
    ...
    assert result.status == "sent"
    assert result.provider_message_id == "gmail-promotion-race"
    assert official_runtime.loads - loads_before == 1
    assert events == [
        "global_guard",
        "adapter_constructed",
...
def _legacy_release_payloads() -> tuple[dict[str, Any], dict[str, Any]]:
    common = {
        ...
        "release_path": (
            "/opt/imperial-intelligence/releases/"
            "architect-source-1111e75-20260829T024000Z"
        ),
...
def test_trusted_proof_field_drift_stays_required_then_fails_closed_zero_provider(
    ...
    if current_state == "removed":
        ...
        official_runtime.current = _Registry(
            {original_source_id: _passive_source("json_api")}
        )
    monkeypatch.setattr(service, "writes_unlocked", lambda: True)
    db.commit()
    ...
    def worker(_index: int) -> str:
        with SessionLocal() as session:
            current = session.scalar(
                select(OutreachMessage).where(OutreachMessage.id == row.id)
            )

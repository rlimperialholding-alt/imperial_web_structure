...
# --- AC-06: a 0073 downgrade DROP-sorrendje child-before-parent ---
...
def _v2_plan(db, plan_id: str, direct_codes: list[str]) -> ProjectFinancePlan:
    ...
    v2 = ProjectFinancePlan(
        plan_id=plan_id, project_id=PROJECT, version=2, status="finance_approved",
    ...
    for code in direct_codes:
        db.add(ProjectFinanceBudgetLine(
            line_id=f"FIN-LINE-{plan_id}-{code}", plan_id_fk=v2.id, cost_code=code,
    ...
    db.add(ProjectFinanceCashflowLine(
        flow_id=f"FIN-FLOW-{plan_id}-IN", plan_id_fk=v2.id,
        period_date=datetime.now(UTC).date(), direction="inflow",
        category="bevétel", description="v2 bevétel", amount_net=Decimal("10000000"),
    ))
    db.add(ProjectFinanceCashflowLine(
        flow_id=f"FIN-FLOW-{plan_id}-OUT", plan_id_fk=v2.id,
...
def test_gate_counts_orphan_commitment_conservatively(db):
    ...
    db.add(FinanceCommitment(
        commitment_id="FCOMMIT-ORPHAN", plan_id_fk=plan.id, cost_code="GHOST-CODE",
...
def test_subcontract_dispatch_transition_is_gated(db):
    ...
    dispatched = record_contract_dispatch(
        db, row.contract_id, _user("legal"),
...
def test_award_then_po_preparation_approval_counts_once(client, db):
    ...
    from tests.test_tender_margin_enforcement import (
        TENDER_ID,
        _bid,
        _login,
        _project,

...
from ..models import EventRecord, ModuleRegistry, OutboxMessage, ProjectObjectState, ProjectRegistry, TaskRecord
...
def update_project_risk(project: ProjectRegistry, event: EventRecord) -> None:
    ...
    if event.severity in {"high", "critical"} and event.status == "open":
        project.blocked = event.event_type in {
            "PAYMENT_BLOCKED", "DELIVERY_NOTE_MISSING", "QUALITY_VARIANCE_DETECTED", "PERFORMANCE_DECLARATION_MISSING",
            "PROJECT_MARGIN_AT_RISK", "MODULE_HEALTH_FAILED",
        }
    project.financial_impact_huf = Decimal(str(project.financial_impact_huf or 0)) + abs(Decimal(str(event.financial_impact_huf or 0)))
    project.deadline_impact_days = max(project.deadline_impact_days or 0, event.deadline_impact_days or 0)
...
def ingest_event(db: Session, event_in: EventIn, *, actor: str = "api") -> tuple[EventRecord, bool]:
    ...
    next_action = event_in.next_action or (definition.task_title if definition.create_task else None)
    ...
    audit(db, actor=actor, action="event_ingested", entity_type="event", entity_id=record.event_id, after=event_in.model_dump(mode="json"))
    db.commit()
    db.refresh(record)
    return record, True


def register_heartbeat(db: Session, heartbeat: HeartbeatIn, *, actor: str = "api") -> ModuleRegistry:
    module = db.scalar(select(ModuleRegistry).where(ModuleRegistry.module_key == heartbeat.module_key))
    if not module:
        module = ModuleRegistry(module_key=heartbeat.module_key, name=heartbeat.module_key.replace("_", " ").title())
        db.add(module)
    ...
    audit(db, actor=actor, action="heartbeat", entity_type="module", entity_id=heartbeat.module_key, after=heartbeat.model_dump())
    db.commit()
    db.refresh(module)
    return module


def process_outbox(db: Session, *, limit: int = 100, simulate_success: bool = True) -> dict[str, int]:
    now = utcnow()
    messages = db.scalars(select(OutboxMessage).where(

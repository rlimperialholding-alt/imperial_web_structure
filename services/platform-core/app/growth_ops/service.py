...
def ingest_signal(
    ...
    *,
    ...
    source = (
        registry_sources.get(row.source_id)
...
def _trusted_official_source_binding_proof_audit(
    ...
    for proof in signal_bound:
        ...
        if (
            ...
            == GrowthRegistry.OFFICIAL_COMPANY_SOURCE_KIND
            and payload.get("source_fetch_mode")
            == GrowthRegistry.OFFICIAL_COMPANY_FETCH_MODE
...
def migrate_official_source_binding_proofs_locked(
    ...
    *,
    ...
    if (
        ...
        or not managed_gate.is_file()
        or writes_unlocked()
    ):
        raise OfficialSourceEvidenceError(
            "official_source_binding_migration_requires_runtime_lock"
        )
    if (
        not targets
        ...
        raise OfficialSourceEvidenceError(
            "official_source_binding_migration_targets_invalid"
        )
    ...
    try:
        ...
        if existing_completion:
            ...
            try:
                existing_payload = json.loads(
                    existing_completion[0].after_json or "{}"
                )
            ...
            existing_hmac = str(
                existing_payload.pop("migration_hmac_sha256", "")
            )
            if (
            ...
            audit(
                ...
                after={
                    **completion_payload,
                    "migration_hmac_sha256": _official_source_receipt_hmac(
                        completion_payload
                    ),
                },
            )
        if not runtime_marker.is_file() or writes_unlocked():
            raise OfficialSourceEvidenceError(
                "official_source_binding_migration_lock_lost"
            )
        db.commit()
        return proof_ids
    except Exception:
...
def _official_source_required(
    ...
    return bool(
        ...
        or _trusted_official_source_binding_proof_audit(db, row, signal)
        is not None
    )
...
def _assert_official_source_provenance_consistent(
    ...
    if (
        ...
        raise OfficialSourceEvidenceError(
            "official_source_provenance_binding_changed"
        )
...
def _refresh_official_source_evidence(
    ...
    try:
        ...
        if _current_canonical_screening_values(
            signal, canonical_metadata["render_input"]
        ) != screening_before:
        ...
        audit(
            ...
            after={
                **receipt_payload,
                "receipt_hmac_sha256": _official_source_receipt_hmac(
                    receipt_payload
                ),
            },
        )
...
def _assert_official_source_evidence_fresh(
    ...
    *,
    ...
    if (
        receipt.get("attestation_scheme")
        != "HMAC-SHA256:IMPERIAL_RELEASE_HMAC_KEY:v1"
    ...
    if (
        ...
        or receipt.get("matched_organization_marker")
        not in expected_organization_markers
        ...
        or {str(page.get("requested_url") or "") for page in pages}
        != expected_requested_urls
...
def dispatch_outreach(db: Session, row: OutreachMessage) -> OutreachMessage:
    ...
    if row.lease_expires_at is None or _aware(row.lease_expires_at) <= utcnow():
        return _release_untransported_claim(
            db, row, reason="outreach_claim_lease_expired_no_send"
        )
    signal = db.scalar(
        select(GrowthSignal)
    ...
    try:
        ...
        receipt = SMTPEmailAdapter(binding).send(
            ...
            pre_send_guard=lambda: _assert_outreach_pre_send_guard(
                db,
                row,
                signal,
                official_required=official_required,
        ...
        row.last_error = (
            str(exc)
            if recipient_hard_gate or isinstance(exc, OfficialSourceEvidenceError)
...
def readiness(db: Session) -> tuple[bool, dict[str, Any]]:
    ...
    # A fresh degraded heartbeat still proves that the sole worker process is
    # alive and serving independent guarded lanes. Unrelated content/SLA
    # degradation must not be mislabeled as a missing worker or block outreach
    # ingest; stale or terminal worker states remain fail-closed.
    heartbeat_serving = bool(hb and hb.status in {"healthy", "working", "degraded"})

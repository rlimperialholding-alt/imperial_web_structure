"""Fail-closed SSRF-védelemmel ellátott HTTP kliens a provider-connectorokhoz.
...
def validate_identifier(
    value: object, *, label: str = "azonosító", max_length: int = MAX_IDENTIFIER_LENGTH
) -> str:
    """Betűk, számjegyek, ``-`` és ``_`` karakterekből álló, korlátos azonosító.

    A kapott érték csak akkor kerülhet URL-útvonalba, ha ezen a szűrőn átment;
    így nem tud path-szeparátort, traversal szegmenst vagy kódolt karaktert becsempészni.
    """
    if not isinstance(value, str) or not value or len(value) > max_length:
        raise SafeHttpError(f"Érvénytelen {label}.")
    ...
    def handle_request(self, request: httpx.Request) -> httpx.Response:
        httpcore_request = httpcore.Request(
            ...
            url=httpcore.URL(
                ...
                # a params-argumentummal küldött kérések (pl. Directus fields,
                # Google Business, DPM) query nélkül sosem hagyhatják el a kliens.

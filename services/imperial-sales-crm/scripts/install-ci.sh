...
if [[ "${SITES_SKIP_TARBALL_PREFLIGHT:-0}" != "1" ]]; then
...
if [[ "${use_seeded_cache}" == "0" && "${SITES_SKIP_TARBALL_PREFLIGHT:-0}" != "1" ]]; then
...
elif [[ "${SITES_SKIP_TARBALL_PREFLIGHT:-0}" == "1" ]]; then
  echo "[sites] locked-tarball preflight skipped by the isolated Docker build"
fi

echo "[sites] running exactly one bounded npm ci"
export NPM_CONFIG_MAXSOCKETS="${SITES_NPM_MAXSOCKETS:-1}"
export NPM_CONFIG_FETCH_RETRIES=0
export NPM_CONFIG_FETCH_TIMEOUT=30000
npm_ci_args=(ci --cache "${expected_cache}")
if [[ "${use_seeded_cache}" == "1" || "${SITES_PREFER_OFFLINE:-0}" == "1" ]]; then
